...
def create_database():
    ...
    try:
        ...
        # ----------------------------------------------------
        # LINK APPOINTMENTS TO SCREENINGS
        # ----------------------------------------------------

        if "scan_id" not in appointment_columns:
            cursor.execute(
                """
                ALTER TABLE appointments
            ...
            print("✓ Added scan_id to existing appointments")
        ...
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_screenings_phone
            ON screenings(phone)
            """
        )
        ...
        cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_clinical_reviews_request
            ON clinical_reviews(request_id)
            """
        )
...
# ============================================================
# 15A. CREATE SECOND DEVELOPMENT PROVIDER
# ============================================================

def create_second_provider():

    provider_id = "provider2"
    provider_name = "OcuSphere Retina Centre"
    provider_email = "retina@ocusphere.local"
    default_password = "OcuSphere456!"
    ...
    try:
        ...
        if cursor.fetchone() is not None:
            ...
            print("✓ Second provider account already exists")
            return
        ...
        connection.commit()
        print("✓ Second provider account created")
...
def resolve_provider(
    ...
    provider_text = requested_provider.strip()
    ...
    try:
        ...
        if provider_id_text:
            ...
            cursor.execute(
                """
                SELECT provider_id, provider_name, email, active
                FROM providers
                WHERE provider_id = ? COLLATE NOCASE
                LIMIT 1
                """,
            ...
            cursor.execute(
                ...
                FROM providers
                WHERE (
                    provider_id = ? COLLATE NOCASE
                    OR provider_name = ? COLLATE NOCASE
                )
                LIMIT 1
                """,
...
# ============================================================
# 22A. PUBLIC ACTIVE PROVIDER LIST
# ============================================================
...
def get_active_providers():
    ...
    try:
        ...
        print(
            "Provider list database error:",
            str(error),
        )

        raise HTTPException(
            status_code=500,
            detail="Unable to retrieve healthcare providers.",
        )
    ...
    try:
        ...
        cursor.execute(
            ...
            (
                ...
                appointment.scan_id.strip()
                if appointment.scan_id
                else None,
            ),
        )
...
# ============================================================
# 29A. PROVIDER CLINICAL REVIEW DATA
# ============================================================

@app.get("/provider/clinical-review/{request_id}")
def get_provider_clinical_review(
    request_id: str,
    authorization: Optional[str] = Header(default=None),
):
    provider = get_authenticated_provider(authorization)
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT request_id, patient_name, phone, email, provider,
                   provider_id, preferred_date, preferred_time, reason,
                   notes, status, created_at, scan_id
            FROM appointments
            WHERE UPPER(request_id) = ?
            LIMIT 1
            """,
        ...
        if (appointment.get("provider_id") or "").lower() != provider_id.lower():
            raise HTTPException(
                status_code=403,
                detail="You are not authorised to review this appointment.",
            )

        screening = None

        if appointment.get("scan_id"):
            cursor.execute(
                """
                SELECT scan_id, patient_name, phone, filename, grade,
                       prediction, severity, confidence, recommendation, created_at
                FROM screenings
                WHERE scan_id = ?
                LIMIT 1
                """,
        ...
        # Compatibility for appointments made before scan_id linking:
        # find the latest screening for the same phone and persist that link.
        if screening is None and appointment.get("phone"):
            cursor.execute(
                """
                SELECT scan_id, patient_name, phone, filename, grade,
                       prediction, severity, confidence, recommendation, created_at
            ...
            if screening_row:
                ...
                cursor.execute(
                    ...
                    WHERE UPPER(request_id) = ?
                    """,
                    (screening["scan_id"], request_id),
                )
                connection.commit()
                appointment["scan_id"] = screening["scan_id"]

        cursor.execute(
            """
            SELECT review_id, request_id, scan_id, provider_id,
                   clinical_assessment, recommendation, additional_notes,
        ...
        return {
            ...
            "screening": screening,
            "clinical_review": dict(review_row) if review_row else None,
        }
    finally:
        connection.close()
...
# ============================================================
# 29B. SAVE / UPDATE PROVIDER CLINICAL REVIEW
# ============================================================

@app.put("/provider/clinical-review/{request_id}")
def save_provider_clinical_review(
    request_id: str,
    review: ClinicalReviewRequest,
    authorization: Optional[str] = Header(default=None),
):
    provider = get_authenticated_provider(authorization)
    provider_id = provider["provider_id"]
    ...
    assessment = review.clinical_assessment.strip()
    recommendation = review.recommendation.strip()
    notes = review.additional_notes.strip() if review.additional_notes else ""

    if not assessment:
        raise HTTPException(status_code=400, detail="Clinical assessment is required.")
    if not recommendation:
        raise HTTPException(status_code=400, detail="Clinical recommendation is required.")
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT request_id, provider_id, scan_id
            FROM appointments
            WHERE UPPER(request_id) = ?
            LIMIT 1
            """,
        ...
        if (appointment["provider_id"] or "").lower() != provider_id.lower():
            raise HTTPException(
                status_code=403,
                detail="You are not authorised to review this appointment.",
            )
        ...
        if existing:
            ...
            cursor.execute(
                ...
                SET scan_id = ?, clinical_assessment = ?, recommendation = ?,
                    additional_notes = ?, updated_at = ?
                WHERE UPPER(request_id) = ?
                AND provider_id = ? COLLATE NOCASE
                """,
                (
                    appointment["scan_id"], assessment, recommendation,
                    notes, now, request_id, provider_id,
                ),
            )
        else:
            ...
            cursor.execute(
                ...
                INSERT INTO clinical_reviews (
                    review_id, request_id, scan_id, provider_id,
                    clinical_assessment, recommendation, additional_notes,
                    created_at, updated_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    review_id, request_id, appointment["scan_id"], provider_id,
                    assessment, recommendation, notes, now, now,
                ),
            )
        ...
        return {
            "success": True,
            "message": "Clinical review saved successfully.",
            "clinical_review": dict(cursor.fetchone()),
        }
    finally:
        connection.close()
...
# ============================================================
# 29C. PATIENT CLINICAL REVIEW LOOKUP
# ============================================================

@app.get("/appointments/{request_id}/clinical-review")
def get_patient_clinical_review(request_id: str):
    request_id = request_id.strip().upper()
    connection = get_database_connection()
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT r.review_id, r.request_id, r.scan_id,
                   r.clinical_assessment, r.recommendation,
                   r.additional_notes, r.created_at, r.updated_at,
                   p.provider_name
            FROM clinical_reviews AS r
            LEFT JOIN providers AS p
                ON LOWER(p.provider_id) = LOWER(r.provider_id)
            WHERE UPPER(r.request_id) = ?
            LIMIT 1
            """,
            (request_id,),
        )
        ...
        if row is None:
            raise HTTPException(
                status_code=404,
                detail="No provider clinical review has been recorded for this appointment.",
            )

        return {"success": True, "clinical_review": dict(row)}
    finally:
        connection.close()
...
async def analyse(
    ...
    patient_name: str = Header(default="", alias="X-Patient-Name"),
    patient_phone: str = Header(default="", alias="X-Patient-Phone"),
):
    ...
    try:
        ...
        cleaned_patient_name = patient_name.strip() if patient_name else "Guest Patient"
        cleaned_patient_phone = patient_phone.strip() if patient_phone else ""
        ...
        try:
            ...
            connection.commit()
            print("✓ Screening saved:", scan_id)

        except sqlite3.Error as database_error:
            print("Screening history save error:", str(database_error))
...
# ============================================================
# 30A. GET PATIENT SCREENING HISTORY
# ============================================================

@app.get("/screenings/{phone}")
def get_patient_screenings(phone: str):
    ...
    try:
        ...
        print("Screening history database error:", str(error))
        raise HTTPException(
            status_code=500,
            detail="Unable to retrieve screening history.",
        )
