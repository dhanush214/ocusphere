...
export default function DoctorsPage() {
  ...
  useEffect(() => {
    async function loadRegisteredProviders() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            data?.detail || "Unable to load OcuSphere registered providers."
          );
        }

        setRegisteredProviders(data.providers || []);
      } catch (err) {
        console.error("Registered provider loading error:", err);

        setProvidersError(
          err instanceof Error
            ? err.message
            : "Unable to load OcuSphere registered providers."
        );
      } finally {
  ...
  const bookRegisteredProvider = (provider: RegisteredProvider) => {
            ...
            <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.025] p-8 text-center md:p-10">
              ...
              <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
                ...
                <button
                  onClick={() =>
                    document
                      .getElementById("registered-providers")
        ...
        >
          <div>
            ...
            <p className="mt-3 max-w-3xl leading-7 text-slate-400">
              These providers are connected to the OcuSphere appointment
              system. Requests made here are securely routed to the selected
              provider&apos;s authenticated dashboard.
            </p>
          </div>

          {providersLoading && (
            <div className="mt-7 rounded-3xl border border-white/10 bg-white/[0.025] p-8 text-center">
            ...
            registeredProviders.length === 0 && (
              <div className="mt-7 rounded-3xl border border-white/10 bg-white/[0.025] p-8 text-center">
                <p className="font-semibold text-slate-300">
                  No registered providers are currently available.
                </p>
              </div>
            )}

          {!providersLoading && registeredProviders.length > 0 && (
            <div className="mt-7 grid gap-5 md:grid-cols-2">
              {registeredProviders.map((provider) => (
                <RegisteredProviderCard
                  key={provider.provider_id}
                  provider={provider}
                  onBook={() => bookRegisteredProvider(provider)}
                />
              ))}
            </div>
          )}
...
function RegisteredProviderCard({
  provider,
  onBook,
}: {
  ...
  return (
    <article className="rounded-3xl border border-emerald-400/20 bg-emerald-400/[0.025] p-6 md:p-7">
      <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-emerald-400/20 bg-emerald-400/[0.08] font-bold text-emerald-300">
        ✓
      </div>

      <span className="mt-5 inline-flex rounded-full border border-emerald-400/25 bg-emerald-400/[0.07] px-3 py-1 text-xs font-semibold text-emerald-300">
        Registered OcuSphere Provider
      </span>
      ...
      <p className="mt-5 text-sm leading-6 text-slate-400">
        Appointment requests are routed directly to this provider&apos;s
        authenticated OcuSphere dashboard.
      </p>

      <button
        onClick={onBook}
        className="mt-6 w-full rounded-xl bg-gradient-to-r from-purple-600 via-blue-500 to-cyan-500 px-6 py-3.5 font-semibold transition hover:opacity-90"
      >
        Request Appointment →
      </button>
    </article>
